...
"""
전체 자동화 파이프라인:
1. 네이버금융에서 상한가 + 15~29.4% 급등(거래대금상위 + 등락률상위 통합) 종목 크롤링
2. 종목별 상세정보(업종/시가총액/상장주식수/외국인비율/거래대금/거래량) 크롤링
...
def build_row_for_upper_limit(stock, today_str, enrichment, summary, row_number):
    ...
    return [
        today_str,                                            # A Date
        stock["rank"],                                        # B No.
        stock["continuous_days"],                             # C 연속
        stock["cumulative"],                                  # D 누적
        stock["name"],                                        # E 종목명
        stock["price"],                                       # F 현재가
        f'{stock["change_rate"]}%',                           # G 등락률
        enrichment.get("trading_value_million"),              # H 거래대금(백만)
        enrichment.get("market_cap_eok"),                     # I 시가총액(억)
        f"=(H{row_number}*0.01)/I{row_number}",               # J 수식
        stock["volume"],                                      # K 거래량
        _to_thousands(enrichment.get("listed_shares")),       # L 상장주식수(천)
        f"=(K{row_number}*0.001)/L{row_number}",              # M 수식
        enrichment.get("foreign_ratio"),                      # N 외국인비율
        enrichment.get("industry"),                           # O 업종
        *_pad_keywords(summary.get("keywords", [])),          # P Q R 상승 키워드1~3
        summary.get("issue"),                                 # S 당일 상승 이슈
        summary.get("theme"),                                 # T 테마
    ]


def build_row_for_top_value(stock, today_str, enrichment, summary, row_number):
    """15~29.4% 급등 종목 1개를 시트의 한 행으로 변환."""
    return [
        today_str,                                            # A Date
        "-",                                                  # B No.
        "-",                                                  # C 연속
        "-",                                                  # D 누적
        stock["name"],                                        # E 종목명
        enrichment.get("price"),                              # F 현재가
        f'{stock["change_rate"]}%',                           # G 등락률
        enrichment.get("trading_value_million"),              # H 거래대금(백만)
        enrichment.get("market_cap_eok"),                     # I 시가총액(억)
        f"=(H{row_number}*0.01)/I{row_number}",               # J 수식
        enrichment.get("volume"),                             # K 거래량
        _to_thousands(enrichment.get("listed_shares")),       # L 상장주식수(천)
        f"=(K{row_number}*0.001)/L{row_number}",              # M 수식
        enrichment.get("foreign_ratio"),                      # N 외국인비율
        enrichment.get("industry"),                           # O 업종
...
def main():
    ...
    print("\n=== 2. 15~29.4% 급등 종목 크롤링 (거래대금상위 + 등락률상위 통합) ===")
    top_value_stocks = fetch_notable_gainers(min_rate=15.0, max_rate=29.4)
    ...
    cell_range = f"A{start_row}:T{end_row}"  # A~T: 총 20개 컬럼
    worksheet.update(range_name=cell_range, values=rows, value_input_option="USER_ENTERED")

    # J, M열은 분수 형태의 수식 결과라서 퍼센트(소수점 1자리) 서식 지정
